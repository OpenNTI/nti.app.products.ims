...
class TestToolViews(ApplicationLayerTest):
    ...
    def test_config(self):
        ...
        assert_that(conf.launch_url, is_('http://localhost/dataserver2/IMS/LTI/TOOLS/launch'))
        assert_that(conf.secure_launch_url, is_('http://localhost/dataserver2/IMS/LTI/TOOLS/launch'))

        assert_that(conf.extensions, has_entry('canvas.instructure.com',
    ...
    def test_launch(self):
        ...
        result = self.testapp.post(request.url, headers=request.headers, params=request.body, status=400)

        consumer = None
        with mock_dataserver.mock_db_trans(self.ds):
            consumer = self._register_consumer(u'foonextthoughtcom', u'supersecret')

        _, request = self._make_launch_request(consumer.key, consumer.secret)
        result = self.testapp.post(request.url, headers={}, params=request.body, status=303)
